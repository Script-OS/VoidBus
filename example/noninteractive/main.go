...
import (
	...
	"github.com/Script-OS/VoidBus/channel/quic"
...
type TestRunner struct {
	...
	// QUIC TLS configuration (shared across QUIC tests)
	quicServerTLS *tls.Config
...
// initQUICTLS initializes QUIC TLS configurations (lazy initialization)
func (tr *TestRunner) initQUICTLS() error {
	tr.quicMu.RLock()
	if tr.quicClientTLS != nil {
		tr.quicMu.RUnlock()
		return nil
	}
	tr.quicMu.RUnlock()
	...
	tr.quicMu.Lock()
	defer tr.quicMu.Unlock()

	if tr.quicClientTLS != nil {
		return nil
	}

	// Client TLS config with InsecureSkipVerify for testing
	tr.quicClientTLS = &tls.Config{
		InsecureSkipVerify: true,
		NextProtos:         []string{"voidbus"},
	}

	// Server TLS will be generated by QUIC channel's generateSelfSignedCert()
	tr.quicServerTLS = nil
...
func (tr *TestRunner) createServerChannel(chType channel.ChannelType, port int) (channel.Channel, error) {
	...
	case channel.TypeQUIC:
		...
		// QUIC server channel will generate self-signed certificate internally
		return quic.NewServerChannel(config)
	default:
...
func (tr *TestRunner) createClientChannel(chType channel.ChannelType, port int) (channel.Channel, error) {
	...
	case channel.TypeQUIC:
		...
		if err := tr.initQUICTLS(); err != nil {
			return nil, err
		}
		tr.quicMu.RLock()
		clientTLS := tr.quicClientTLS
		tr.quicMu.RUnlock()
		...
		return quic.NewClientChannel(config)
	default:
		return nil, fmt.Errorf("unknown channel type: %s", chType)
	}
}
